...
const CONTACT_PATHS = ['/contato', '/contact', '/fale-conosco', '/contatos', '/contact-us'];
...
export async function scrapeWebsite(url) {
  ...
  if (!hasForm) {
    const formLinks = $('a[href]').filter(function () {
      ...
      return CONTACT_PATHS.some(p => href.includes(p)) || /contato|contact|fale.?conosco/i.test(href);
    });
