...
export function setExperimentContext(ctx) {
  _experimentContext = {
    ...
    experiment_variant_id: ctx?.experiment_variant_id ?? null,
  };
}
...
const LEAD_COLUMNS = [
  ...
  "owner_probability",
  "outreach_variant",
  // Lab — see fastdevbuilds-admin migration 20260430_experiments_lab.sql
  "experiment_id",
...
function prepareRow(lead) {
  ...
  if (row.experiment_id == null && _experimentContext.experiment_id != null) {
    row.experiment_id = _experimentContext.experiment_id;
  }
  if (
    row.experiment_variant_id == null &&
    _experimentContext.experiment_variant_id != null
  ) {
    row.experiment_variant_id = _experimentContext.experiment_variant_id;
  }
