...
/**
 * Fetches all distinct (niche, search_city) pairs that already exist in the leads table.
...
async function getProspectedCombos() {
  ...
  // Supabase doesn't support SELECT DISTINCT on multiple columns natively,
  // so we fetch niche + search_city from all leads and deduplicate in memory.
  // We paginate in chunks of 1000 to handle large tables.
 ...
 *   stats: { total, prospected, remaining, whatsappSentToday, whatsappSlotsLeft }
 * }
 */
export async function generateQueue() {
  ...
  return {
    ...
    stats: {
      ...
      prospected: prospected.size,
      remaining: queue.length,
      whatsappSentToday: sentToday,
      whatsappSlotsLeft: Math.max(0, 50 - sentToday),
    },
  };
}
