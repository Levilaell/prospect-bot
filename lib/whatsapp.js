...
// Fallback daily cap applied when an instance has no row in
// evolution_instance_config (should not happen for configured instances,
// but keeps the bot running if someone adds an env var without seeding DB).
...
/** Daily caps per instance, loaded once per run from Supabase. */
const _dailyCaps = {};

/**
 * Fetch daily_cap values from evolution_instance_config.
 * Called once at the start of sendWhatsApp(). Populates _dailyCaps.
 */
async function loadDailyCaps(instances) {
  ...
  if (error) {
    console.warn(`⚠️   Failed to load daily caps (${error.message}) — using fallback ${FALLBACK_DAILY_CAP} for all`);
    for (const n of names) _dailyCaps[n] = FALLBACK_DAILY_CAP;
    return;
  }

  const seen = new Set();
  for (const row of data ?? []) {
    _dailyCaps[row.instance_name] = row.daily_cap;
    seen.add(row.instance_name);
  }
  for (const n of names) {
    if (!seen.has(n)) {
      console.warn(`⚠️   No config row for '${n}' — using fallback ${FALLBACK_DAILY_CAP}`);
      _dailyCaps[n] = FALLBACK_DAILY_CAP;
    }
  }
}

/**
 * Returns next instance in round-robin that:
 *   1. Hasn't hit its daily cap (from _dailyCaps / evolution_instance_config)
 *   2. Hasn't hit its run cap (if maxThisRun was provided)
 *
 * Returns null if all instances are capped. Caller must treat null as
 * "capacity exhausted this run — stop".
 */
function nextInstance() {
  const instances = getInstances();
  if (instances.length === 0) throw new Error('No Evolution API instances configured');

  // Try up to `instances.length` times to find one with remaining capacity
  for (let tries = 0; tries < instances.length; tries++) {
    ...
    const dailyCap = _dailyCaps[inst.name] ?? FALLBACK_DAILY_CAP;
    const runCap = inst.maxThisRun;  // undefined if no run cap

    const belowDaily = sentSoFar < dailyCap;
    const belowRun = runCap == null || sentSoFar < runCap;

    if (belowDaily && belowRun) return inst;
  }
...
export async function sendWhatsApp(leads, { maxSend } = {}) {
  ...
  // Load daily caps from evolution_instance_config BEFORE initializing counters
  await loadDailyCaps(instances);

  // Initialize per-instance counters from DB (counts today's sends)
  ...
  for (let i = 0; i < toSend.length; i++) {
    ...
    if (!instance) {
      const status = getInstances().map(inst => {
        ...
        const daily = _dailyCaps[inst.name] ?? FALLBACK_DAILY_CAP;
        const run = inst.maxThisRun;
        const reason = sent >= daily
          ? `daily ${sent}/${daily}`
          : (run != null && sent >= run ? `run ${sent}/${run}` : 'ok');
