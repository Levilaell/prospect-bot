...
export async function sendWhatsApp(leads) {
  ...
  for (let i = 0; i < toSend.length; i++) {
    ...
    try {
      await sendMessage(phone, lead.message, instance);
      await markSent(lead.place_id, instance.name);
      ...
      console.warn(`\n⚠️   WhatsApp send failed for ${lead.business_name} (${instance.name}): ${err.message}`);
      failed++;
    }
