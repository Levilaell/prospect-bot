...
// + Getimg (up to 6 images) + Supabase upload. Observed times spread from
// 60s to 4 min; 300s matches Vercel's maxDuration cap on the route, so the
// bot never gives up before the function itself does. Admin returns
// already_existed on retry if the work happened to race past our abort.
const TIMEOUT_OVERRIDES = {
  '/api/bot/project/create': 300_000,
};
