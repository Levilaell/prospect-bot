...
/**
 * Interpolate {nome}, {negocio}, {cidade}, {vertical} placeholders in a
 * variant template. Missing fields render as empty string — caller is
 * responsible for picking variants whose templates make sense without
 * optional fields.
 *
 * {nome} and {negocio} are aliases for business_name. Cold leads from
 * Google Places don't carry the owner's personal name, so both
 * placeholders resolve to the same business_name string.
 */
export function interpolateMessageTemplate(template, lead) {
  if (!template) return '';
  return template
    .replace(/\{nome\}/g, lead.business_name ?? '')
    .replace(/\{negocio\}/g, lead.business_name ?? '')
